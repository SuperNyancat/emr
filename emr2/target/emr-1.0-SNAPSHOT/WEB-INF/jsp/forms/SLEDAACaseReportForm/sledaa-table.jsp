...
<html lang="en">
<head>
	<title>SLEDAA Case Report Records</title>
	<meta charset="utf-8">
...
<c:choose>
	...
	<ul class="ctrlBtn">
		<li><span class="btn"><a href="SLEDAA_case_report_form_assembler.it?id=${patient.getId() }"><input type="button" value="New Record" class="btnNew"></a></span></li>
...
<div class="content-wrap">	
	<div class="wrapper prospects">
		...
		<div style="background:goldenrod">
				<br><p class="titles" style="color:white;">Systematic Lupus Erythematosus Disease Activity Assessments (SLEDAA) Case Report Form</p><br>		
		</div>
			<div class="tableSearch clrfix">
				<section>
					   <form:form commandName="patient" action="search_sledaa_case_report.it" method="GET">
				...
				<table class="tablecontent3">
						...
						<tr>
							...
							<th class="phn">MEX SLEDAI Score</th>
							<th class="ind">SLICC Damage Score</th>
							<th class="ind">Treatment of Lupus Activity</th>				
						</tr>
		...
		<tr>
			...
			<c:forEach items="${sledaaForms }" var="form">
			<tr>
				<td><div class="blue"></div></td>
				<td class="pro"><a href="view_sledaa.it?pid=${patient.getId()}&fid=${form.getId()}">${ form.getRevisionHistory().getDateCreated()}</a></td>
				
				<td class="phn">${form.getMEXSLEDAIScore() }</td>				
				<td class="ind">${form.getSLICCDamageScore() }</td>
				<c:choose>
					<c:when test="${form.getTreatment().isEmpty()}">
						<td class="ind">None</td>
					</c:when>
							
					<c:otherwise>
						<td class="ind">${form.getTreatment() }</td>		
					</c:otherwise>							
				</c:choose>
				</tr>
			</c:forEach>
