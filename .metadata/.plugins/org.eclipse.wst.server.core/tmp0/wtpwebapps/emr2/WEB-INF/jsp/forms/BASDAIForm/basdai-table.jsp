...
<c:choose>
	...
	<ul class="ctrlBtn">
		<li><span class="btn"><a href="BASDAI_form_assembler.it?id=${patient.getId() }"><input type="button" value="New Record" class="btnNew"></a></span></li>
...
<div class="content-wrap">	
	<div class="wrapper prospects">
		...
		<div style="background:goldenrod">
				<br><p class="titles" style="color:white;">Bath Ankylosing Spondylitis Disease Activity Index (BASDAI)</p><br>		
		</div>
			<div class="tableSearch clrfix">
				<section>
				
				
				 <form:form commandName="patient" action="search_basdai.it" method="GET">
...
<table class ="tablecontent3">	
		...
		<tr>
			...
			<tr>
				...
				<td class="pro"><a href="view_basdai.it?pid=${patient.getId()}&fid=${form.getId()}">${ form.getRevisionHistory().getDateCreated()}</a></td>
				
				<td class="phn">${form.getScore() }</td>
				
				<c:choose>
					<c:when test="${form.getASMedication().getBrandName() == null}">
						<td class="ind">None</td>
					</c:when>
					<c:otherwise>
						<td class="ind">${form.getASMedication().getBrandName() }</td>
					</c:otherwise>
				</c:choose>		
			</tr>
			</c:forEach>
